    ...
    <v-container fluid>
      <v-row no-gutters>
        <v-col cols="12">
          ...
          >
            <div class="test" style="width: 600px">
              <div v-for="(item, i) in items" :key="i">
                ...
                <div
                  v-else-if="item.send === 'YOU'"
                  class="px-5"
                  style="width: 100%"
                >
                  <p v-if="item.count === 1">고양이</p>
                  <p
                    class="px-4 py-2"
                    style="
                      font-size: 18px;
          ...
          <v-btn @click="testBroadcast">broadcast test</v-btn>
        </v-col>
      </v-row>
    </v-container>
  </v-card>
...
export default {
  ...
  methods: {
    ...
    async send() {
      this.sendCount += 1
      await this.items.push({ data: this.message, send: 'ME' })
    ...
    testBroadcast() {
      this.socket.emit('nuxt', this.message)
    },
  },
}
</script>
